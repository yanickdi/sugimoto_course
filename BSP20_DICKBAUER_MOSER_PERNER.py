...
def simulate_replace_all():
    ...
    while remain_hours >= 0:
        ...
        print('Remaining hours: {:.2f}h'.format(remain_hours))
        ...
        if min_tube_lifetime >= remain_hours:
            print('No more maintenance required till the end of simulation\n')
            break
    ...
    print('Number of shutdowns: {}'.format(shutdown_counter))
    
def simulate_replace_one():
    print('Start simulation of replacing only the broken tubes:')
    vacuum_tube_list = []
    remain_hours = SIMULATION
    shutdown_counter = 0
    costs = 0
    while remain_hours >= 0:
        # generate 4 new tubes at start, and 1 in the following iterations:
        ...
        # reduce the lifetime of all tubes:
